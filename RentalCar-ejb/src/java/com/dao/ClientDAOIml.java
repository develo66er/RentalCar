...
public class ClientDAOIml implements ClientDAO {
            ...
            while (rs.next()) {
                ...
                while (rs2.next()) { // чтения строки результата выполнения запроса
                    ...
                    client.getListItem(i).add(new Car(rs2.getString("Model"),
                            ...
                            rs2.getDouble("Price"), rs2.getInt("Count"), rs2.getInt("RentalId")));
    ...
    // метод добавления данных машины в базу
    @Override
    public boolean addCar(Car car) {
        String insertSql = "INSERT INTO Car (Model,Manufacturer,Price,Count,RentalId) VALUES (?,?,?,?,?)";
        int i = -1;
        try (
                Connection conn = this.sqlConnector.connect();
            ...
            ps.setInt(4, car.getCount());
            ps.setInt(5, car.getRentalId());
            i = ps.executeUpdate();
            conn.commit();
        } catch (Exception e) {
        }
